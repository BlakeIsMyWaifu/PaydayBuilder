import actions, { ChangeSkillStateAction } from 'actions/skillsAction'
...
const skills = (state = defaultstate, action: Record<'type' | 'payload', any>): SkillsState => {
	...
	const changeSkillState = ({ tree, subtree, skill, oldLevel, direction }: ChangeSkillStateAction): SkillsState => {
	...
	const resetTree = (tree: TreeNames): SkillsState => {
		return {
	...
	switch (action.type) {
		case getType(actions.changeSkillState):
			return changeSkillState(action.payload)
		case getType(actions.resetSkills):
			return defaultstate
		case getType(actions.resetTree): {
			return resetTree(action.payload)
		}
		default:
			return state
	}
