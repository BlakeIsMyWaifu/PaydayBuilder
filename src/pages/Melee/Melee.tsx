...
import { EquipedLabel, IconWrapper, MeleeIcon, NameLabel, TableData, TableHead, TableLabel, TableRow, TableStats } from './Melee-Elements'
	...
	return (
		<Container columns='3fr 1fr' rows='4rem 8fr 4rem' areas='"title title" "wrapper info" "wrapper back"'>
			...
			<Wrapper>
				{
					data.map(melee => {
						return <IconWrapper key={melee.name}>
							{
								melee.name === equipedMelee.name && <EquipedLabel>Equiped</EquipedLabel>
							}
							<NameLabel color={itemColours[melee.sourceType]}>{melee.name}</NameLabel>
							<MeleeIcon
